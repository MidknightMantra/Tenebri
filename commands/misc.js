    ...
    switch (sub) {
      ...
      case "namecard": {
        ...
        if (!username || !birthday) {
          await sock.sendMessage(
            chatId,
            {
              text: "Usage: .misc namecard username|birthday|description(optional)",
            },
            { quoted: message },
          );
          return;
      ...
      case "tweet": {
        ...
        if (!displayname || !username || !comment) {
          await sock.sendMessage(
            chatId,
            {
              text: "Usage: .misc tweet displayname|username|comment|theme(optional light/dark)",
            },
            { quoted: message },
          );
          return;
      ...
      default:
        await sock.sendMessage(
          chatId,
          {
            text: "Usage: .misc <heart|horny|circle|lgbt|lesbian|nonbinary|pansexual|transgender|lied|lolice|simpcard|tonikawa|its-so-stupid <text>|namecard u|b|d?|nobitches <text>|oogway <q>|oogway2 <q>|tweet dn|un|c|theme?|youtube-comment un|c>",
          },
          { quoted: message },
        );
        break;
