...
async function facebookCommand(sock, chatId, message) {
  try {
    ...
    if (data && data.status && data.data && Array.isArray(data.data.data)) {
      ...
      const hdVideo = data.data.data.find(
        (item) => item.resolution === "HD" && item.format === "mp4",
      );
      const sdVideo = data.data.data.find(
        (item) => item.resolution === "SD" && item.format === "mp4",
      );
